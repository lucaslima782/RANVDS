...
try:
    ...
    # Fallback when running from bundled/local sources without installed wheel
    __version__ = "local"

if os.name != 'nt':
    # Be tolerant in frozen/child processes where signal registration may fail
    try:
...
def scat_main():
    ...
    for parser_module in dir(scat.parsers):
        if parser_module.startswith('__'):
            continue
        attr = getattr(scat.parsers, parser_module)
