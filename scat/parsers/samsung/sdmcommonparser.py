...
class SdmCommonParser:
    def __init__(self, parent, icd_ver=(0, 0)):
        ...
        self.process = {
            ...
            g | c.NR_RRC_INFO: lambda x: self.sdm_parse_nr_rrc(x, 0x08),
            g | c.NR_NAS_INFO: lambda x: self.sdm_parse_nr_nas(x, 0x09),
            g | c.NR_PHY_INFO: lambda x: self.sdm_parse_nr_phy(x, 0xF1),     
        }

    def set_icd_ver(self, version):
        self.icd_ver = version

    def _parse_sdm_nr_rrc_message(self, sdm_pkt_hdr, subtype, flag, seq, length, msg):
        rrc_subtype = {
            0: util.gsmtapv3_nr_rrc_types.DL_CCCH, # Correto
            1: util.gsmtapv3_nr_rrc_types.PCCH, #Correto
            2: util.gsmtapv3_nr_rrc_types.BCCH_BCH,#Correto?
            3: util.gsmtapv3_nr_rrc_types.BCCH_DL_SCH, #Correto 
            4: util.gsmtapv3_nr_rrc_types.DL_DCCH, #Correto 
            256: util.gsmtapv3_nr_rrc_types.UL_CCCH, # Correto
#            7: util.gsmtapv3_nr_rrc_types.UL_CCCH1, 
            260: util.gsmtapv3_nr_rrc_types.UL_DCCH, #Correto
#            5: util.gsmtapv3_nr_rrc_types.MCCH, 
#            10: util.gsmtapv3_nr_rrc_types.SBCCH_SL_BCH,
#            11: util.gsmtapv3_nr_rrc_types.SCCH,
            } 

        gsmtapv3_metadata = dict()
        gsmtapv3_metadata[util.gsmtapv3_metadata_tags.BSIC_PSC_PCI] = self.parent.nr_last_pci
        gsmtapv3_subtype = rrc_subtype[subtype]
        gsmtapv3_nrarfcn = self.parent.nr_last_nrarfcn_dl

        gsmtap_hdr = util.create_gsmtap_header(
            version = 3,
            payload_type = util.gsmtapv3_types.NR_RRC,
            arfcn = gsmtapv3_nrarfcn,
            sub_type = gsmtapv3_subtype,
            metadata=gsmtapv3_metadata
            )
        return {'layer': 'rrc', 'cp': [gsmtap_hdr + msg]}

    def sdm_parse_nr_rrc(self, pkt, cmdid):
        ...
        if len(pkt) < 7:
            ...
            return None
        header = namedtuple('SdmLteTestDummy', 'flag seq subtype length')
        rrc_header = header._make(struct.unpack('<HBHH', pkt[0:7]))
        rrc_msg = pkt[7:]

        return self._parse_sdm_nr_rrc_message(sdm_pkt_hdr, rrc_header.subtype, rrc_header.flag, rrc_header.seq, rrc_header.length, rrc_msg)

    def _parse_sdm_nr_nas_message(self, sdm_pkt_hdr, direction, length, subtype, message, msg):
        gsmtap_subtype = {
            0x00: util.gsmtapv3_nr_nas_types.NAS5GS_PLAIN,
            0x01: util.gsmtapv3_nr_nas_types.NAS5GS_SEC_HEADER,
            }
        if subtype not in gsmtap_subtype:
            if self.parent:
                self.parent.logger.log(logging.WARNING, 'Unknown NAS subtype ({})'.format(subtype))
            return None
        gsmtapv3_metadata = dict()
        gsmtapv3_metadata[util.gsmtapv3_metadata_tags.BSIC_PSC_PCI] = self.parent.nr_last_pci
        gsmtapv3_subtype = gsmtap_subtype[subtype]
        gsmtapv3_nrarfcn = self.parent.nr_last_nrarfcn_dl
    ...
    def sdm_parse_nr_nas(self, pkt, cmdid):
        sdm_pkt_hdr = sdmcmd.parse_sdm_header(pkt[1:15])
        pkt = pkt[15:-1]
        ...
        if len(pkt) < 5:
            if self.parent:
                self.parent.logger.log(logging.WARNING, 'Packet length ({}) shorter than expected (4)'.format(len(pkt)))
            return None
        header = namedtuple('SdmLteTestDummy', 'direction lenght subtype message')
        nas_header = header._make(struct.unpack('<BHBB', pkt[0:5]))
        nas_msg = pkt[5:]

        return self._parse_sdm_nr_nas_message(sdm_pkt_hdr, nas_header.direction, nas_header.lenght, nas_header.subtype, nas_header.message, nas_msg)

    def sdm_parse_nr_phy(self, pkt, cmdid):
        sdm_pkt_hdr = sdmcmd.parse_sdm_header(pkt[1:15])
        pkt = pkt[15:-1]
        header = namedtuple('SdmLtePhyCellInfo', 'cmd zero1 dk zero2 nrarfcn dk2 zero3 pci zerO4 rsrp rsrq sinr rssi')
        struct_format = '<HHHHIHHHHiiii'
        expected_len = struct.calcsize(struct_format)
        
        # Debug logging
        if self.parent:
            self.parent.logger.log(logging.DEBUG, f'NR PHY packet length: {len(pkt)} bytes, expected: {expected_len} bytes')
            self.parent.logger.log(logging.DEBUG, f'NR PHY packet data: {pkt.hex()}')
            
        if len(pkt) < expected_len:
            return None
            
        try:
            nr_phy_header = header._make(struct.unpack(struct_format, pkt[0:expected_len]))
        except struct.error as e:
            if self.parent:
                self.parent.logger.log(logging.ERROR, f'Failed to unpack NR PHY data: {str(e)}')
            return None
            
        extra = pkt[expected_len:]
        if nr_phy_header.cmd != 0x1F:
            return None
        if len(pkt) < expected_len:
            if self.parent:
        ...
        if self.parent:
            self.parent.nr_last_nrarfcn_dl= nr_phy_header.nrarfcn
            self.parent.nr_last_pci= nr_phy_header.pci

        stdout = 'NR PHY Cell Info: NRRFCN: {}, PCI: {}, RSRP: {:.2f}, RSRQ: {:.2f}, SINR: {:.2f}, RSSI: {:.2f}'.format(nr_phy_header.nrarfcn, nr_phy_header.pci, nr_phy_header.rsrp / 100.0, nr_phy_header.rsrq / 100.0, nr_phy_header.sinr / 100.0, nr_phy_header.rssi / 100.0)
